...
const GO_API_URL = process.env.NEXT_PUBLIC_GO_API_URL || '/api/go';
...
function faviconUrl(tech, size = 32) {
...
// ─── Service Definitions ─────────────────────────────────────────────────────
const SERVICES = [
  { id: 'landing', name: 'Landing', tech: 'Next.js', route: '/', color: '#888', kind: 'frontend' },
  { id: 'dashboard', name: 'Dashboard', tech: 'Next.js', route: '/dashboard', color: '#888', kind: 'frontend' },
  { id: 'docs', name: 'Docs', tech: 'Docusaurus', route: '/docs', color: '#f5a623', kind: 'frontend' },
  { id: 'flask-api', name: 'Core API', tech: 'Flask', route: '/api', color: '#3b82f6', kind: 'backend' },
  { id: 'go-api', name: 'Analytics API', tech: 'Go', route: '/api/go', color: '#00c853', kind: 'backend' },
];
...
const ENDPOINT_GROUPS = [
  {
    ...
    baseUrl: API_URL,
    endpoints: [
      { id: 'flask-root', method: 'GET', path: '/api', name: 'API Root', description: 'Returns service information and list of available endpoints.', params: [], body: null },
      { id: 'flask-health', method: 'GET', path: '/api/health', name: 'Health Check', description: 'Returns health status of the Flask API service.', params: [], body: null },
      { id: 'flask-revenue', method: 'GET', path: '/api/revenue', name: 'Monthly Revenue', description: 'Returns monthly revenue data for the current year with growth percentages.', params: [], body: null },
      { id: 'flask-revenue-summary', method: 'GET', path: '/api/revenue/summary', name: 'Revenue Summary', description: 'Aggregated revenue metrics including total revenue, MRR, ARR, net profit, and runway.', params: [], body: null },
      { id: 'flask-expenses', method: 'GET', path: '/api/expenses', name: 'Expenses', description: 'Breakdown of expenses by category with percentages.', params: [], body: null },
      { id: 'flask-transactions', method: 'GET', path: '/api/transactions', name: 'Transactions', description: 'List of recent transactions with amounts, types, and dates.', params: [{ name: 'limit', type: 'number', default: 10, description: 'Max number of transactions' }], body: null },
      { id: 'flask-projections', method: 'GET', path: '/api/projections', name: 'Projections', description: 'Revenue projections for next quarter and year based on recent growth trends.', params: [], body: null },
      { id: 'flask-add-revenue', method: 'POST', path: '/api/revenue', name: 'Add Revenue', description: 'Create a new revenue entry.', params: [], body: JSON.stringify({ amount: 5000, source: 'subscription' }, null, 2) },
      { id: 'flask-add-expense', method: 'POST', path: '/api/expenses', name: 'Add Expense', description: 'Create a new expense entry.', params: [], body: JSON.stringify({ amount: 2500, category: 'engineering' }, null, 2) },
    ],
  },
  {
    name: 'Go API',
    ...
    baseUrl: GO_API_URL,
    endpoints: [
      { id: 'go-root', method: 'GET', path: '/api/go', name: 'API Root', description: 'Returns service information and list of available endpoints.', params: [], body: null },
      { id: 'go-health', method: 'GET', path: '/api/go/health', name: 'Health Check', description: 'Returns health status of the Go API service.', params: [], body: null },
      { id: 'go-realtime', method: 'GET', path: '/api/go/analytics/realtime', name: 'Real-time Analytics', description: 'Live metrics including active users, request rate, CPU/memory usage, and error rates.', params: [], body: null },
      { id: 'go-risk', method: 'GET', path: '/api/go/analytics/risk', name: 'Risk Analysis', description: 'Monte Carlo simulation-based risk assessment with VaR calculation and risk factors.', params: [], body: null },
      { id: 'go-forecast', method: 'GET', path: '/api/go/analytics/forecast', name: 'Revenue Forecast', description: 'ARIMA model revenue predictions with confidence intervals for 12 months.', params: [], body: null },
      { id: 'go-benchmark', method: 'GET', path: '/api/go/analytics/benchmark', name: 'Industry Benchmark', description: 'Company metrics vs. industry medians with percentile rankings.', params: [], body: null },
    ],
  },
];
...
const ALL_ENDPOINTS = ENDPOINT_GROUPS.flatMap(g =>
  g.endpoints.filter(e => e.method === 'GET').map(e => ({
    ...e, service: g.name, tech: g.tech, color: g.color, runtime: g.runtime,
  }))
);
    ...
    <aside style={{
      ...
      <div style={{ padding: '20px 20px 16px', borderBottom: '1px solid var(--border)' }}>
        <a href="/" style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{
            display: 'inline-flex', width: 28, height: 28, background: '#fff', borderRadius: 6,
            alignItems: 'center', justifyContent: 'center', color: '#000', fontWeight: 800, fontSize: 13,
          }}>$</span>
          <div>
            <div style={{ fontWeight: 700, fontSize: 14, letterSpacing: '-0.02em' }}>MoneyMachine</div>
      ...
      <div style={{ padding: '0 10px 16px', borderTop: '1px solid var(--border)', paddingTop: 16 }}>
        ...
        {SERVICES.map(s => (
          <div key={s.id} style={{
            ...
            <StatusDot status={s.id === 'dashboard' ? 'healthy' : (serviceHealth[s.id] || 'checking')} size={6} />
      ...
      <div style={{
        ...
        <a href="/docs" style={{ color: 'var(--gray-600)', transition: 'color .15s' }}>Docs</a>
        <span style={{ marginLeft: 'auto', color: 'var(--gray-800)' }}>5 services · 1 URL</span>
      </div>
    </aside>
  );
}
    ...
    <div className="fade-in" style={{ padding: '24px 32px', maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: 'var(--gray-600)', marginTop: 4 }}>
            How this project connects 5 services through a single Vercel URL
          </p>
        </div>
        <button
      ...
      <div style={{
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: `repeat(${frontends.length}, 1fr)`, gap: 10 }}>
            {frontends.map(s => (
              <div key={s.id} style={{
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: `repeat(${backends.length}, 1fr)`, gap: 10 }}>
            {backends.map(s => (
              <div key={s.id} style={{
                display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px',
          ...
          <div style={{
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: 'var(--gray-600)', fontFamily: 'var(--font-mono)' }}>
                Python · <span style={{ color: '#3b82f6' }}>/api/*</span>
              </div>
            </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: 'var(--gray-600)', fontFamily: 'var(--font-mono)' }}>
                Go / Gin · <span style={{ color: '#00c853' }}>/api/go/*</span>
              </div>
            </div>
      ...
      <div style={{
        ...
        5 services · 3 runtimes · 1 domain — all orchestrated by Vercel
      </div>
    </div>
  );
}
      ...
      {results && (
        <div className="fade-in">
          ...
          <div style={{ display: 'flex', gap: 12, marginBottom: 24 }}>
            <div style={{ flex: 1, padding: '16px 20px', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 10 }}>
            ...
            <div style={{ flex: 1, padding: '16px 20px', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 10 }}>
              <div style={{ fontSize: 10, color: 'var(--gray-500)', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 600, marginBottom: 6 }}>Comparison</div>
              <div style={{ fontSize: 28, fontWeight: 700, fontFamily: 'var(--font-mono)' }}>
                {flaskAvg > 0 && goAvg > 0 ? <>{(Math.max(flaskAvg, goAvg) / Math.min(flaskAvg, goAvg)).toFixed(1)}<span style={{ fontSize: 14, color: 'var(--gray-600)' }}>x</span></> : '—'}
              </div>
              <div style={{ fontSize: 11, color: 'var(--gray-600)', marginTop: 2 }}>
                {flaskAvg > goAvg ? 'Go is faster' : goAvg > flaskAvg ? 'Flask is faster' : 'Equal'}
              </div>
            </div>
            <div style={{ flex: 1, padding: '16px 20px', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 10 }}>
...
export default function DashboardPage() {
  ...
  const checkHealth = useCallback(async () => {
    const checks = [
      { id: 'landing', url: '/' },
      { id: 'docs', url: '/docs' },
      { id: 'flask-api', url: `${API_URL}/health` },
      { id: 'go-api', url: `${GO_API_URL}/health` },
    ];
    setServiceHealth(prev => ({ ...prev, dashboard: 'healthy' }));
